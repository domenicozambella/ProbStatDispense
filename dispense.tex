...
\begin{theorem}
  La somma di v.a.i.\@  con distribuzione ${\rm N}(\mu,\sigma^2)$ è ancora una v.a.\@ con distribuzione ${\rm N}(\mu,\sigma^2)$.
\end{theorem}
...
Si verifica che (approssimativamente) $B_0,B_1\sim T(n-2)$.
Le rispettive deviazioni standard $s_0,s_1$ si stimano con le formule

\ceq{\hfill s^2_0}{=}{\frac{n}{ n - 2}\sum_{i=1}^n (e_i)^2}

\ceq{\hfill s^2_1}{=}{\frac{s^2_0}{\displaystyle\sum_{i=1}^n (x_i-\bar x)^2}}

dove $e_i$ e $\bar x$ sono definiti come nel paragrafo precedente.

Un test di ipetesi frequentemente fatto in questo contesto è $H_0:\beta_1=0$, $H_A:\beta_1\neq0$.
...
Il modello quindi è

\ceq{1.\hfill y_i}{=}{\beta_0+\beta_1 x_i+\beta_2 z_i+e_i}

Più in generale potermmo avere $k$ predittori diversi.
Per chiarezza discutiamo il caso $k=2$.
...
Questa soluzione permette anche di trattare il caso non lineare.
...
(Testo ripetuto da sopra~\ref{Z-test}.) Si sospetta che una certa terapia faccia aumentare la pressione diastolica. Nella popolazione generale la pressione diastolica ha distribuzione ${\rm N}(\mu_0,\sigma^2)$ con $\mu_0=75$ e $\sigma=5.5$. Assumiamo che tra i pazienti in terapia la pressione diastolica sia distribuita normalmente con media ignota $\mu$ e con la stessa deviazione standard della popolazione generale. 
...
$\alpha\ =\ \Pr\big(Y\ge x_\alpha\ \mathbin\big|\ \mu=\mu_0\big)\ =\ \Pr(X\ge x_\alpha)$, dove $X\sim {\rm N}(\mu_0,\sigma^2/n)$ \medskip

$\phantom{\alpha\ }\ =1-\Pr\bigg(Z\le \dfrac{x_\alpha-\mu_0}{\sigma/\sqrt n}\bigg)$, dove $Z\sim {\rm N}(0,1)$.
