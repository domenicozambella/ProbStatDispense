...
\bigskip
N.B. È facile progettare un test che minimizza una tra $\Pr(T|H_0)$ e $\Pr(\neg T|H_A)$. 
In un caso estremo: un test che a prescindere dai dati rifiuta sempre $H_0$ non fa errori del I tipo, $\Pr(T| H_0)=0$.
Invece un test che non rifiuta mai $H_0$ non fa errori del II tipo,  $\Pr(\neg T|H_0)=0$. 
La difficoltà nel progettare il test è trovare il giusto equilibrio tra i due errori.
...
Chiamiamo\ questa\ probabilit\`a \emph{livello di confidenza.}
Spesso la indicheremo come $1-\alpha$.

Ora possiamo calcolare $\alpha$ dato $\epsilon$. E viceversa possiamo calcolare $\epsilon$ dato $\alpha$.

Per esempio usando le funzioni di R che calcolano i quantili e la funzione di ripartizione otteniamo $\varepsilon=-(s/\sqrt n)\,{\tt qnorm}(\alpha/2)$ oppure $\alpha=2\,{\tt pnorm}(-\varepsilon\sqrt n/s)$.
...
Per esempio usando le funzioni di R che calcolano i quantili e la funzione di ripartizione otteniamo $\varepsilon=-(s/\sqrt n)\,{\tt qt}(\alpha/2,\, n-1)$ oppure $\alpha=2\,{\tt pt}(-\varepsilon\sqrt n/s,\, n-1)$.
