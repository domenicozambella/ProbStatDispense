...
Immagimiamo gli elementi $\omega\in\Omega$ dello spazio campionario come i possibili \emph{risultati\/} di un rilevamento, un esperimento, un sorteggio, ecc. [\textit{outcomes of a trial or of an experiment or individuals of a pupulation\/}]. 
I sottoinsiemi $E\subseteq\Omega$ verranno chiamati \emph{eventi.}


\begin{void}
  In molti casi solo particolari sottoinsiemi di $\Omega$ vengono considerati eventi. 
...
\item[3.] $\Pr(E_1\cup E_2)=\Pr(E_1)+\Pr(E_2)$ per ogni coppia $E_1,E_2$ di eventi \emph{disgiunti}, ovvero $E_1\cap E_2=\0$. Si dice anche che $E_1$ e $E_2$ sono \emph{mutualmente esclusivi}.
\end{itemize}
...
\begin{void}
\begin{itemize}
\item[3$'$.] $\displaystyle\Pr\bigg(\bigcup_{i=1}^{\infty}E_i\bigg)=\sum_{i=1}^\infty\Pr(E_i)$ \ \ per ogni sequenza $E_1,E_2,\ldots\subseteq\Omega$ di eventi mutualmente esclusivi.
\end{itemize}
\end{void}
...
%%%%%%%%%%%%
%%%%%%%%%%%%
\hfill\rlap{.}\clearpage\section{Distribuzione di probabilità discrete e continue}
...
In generale definiamo la \emph{funzione quantile\/} o  \emph{percentile.} Questa è la funzione che dato $p\in(0,1)$ ritorna quel valore $x=Q_X(p)$ tale che $p\le\Pr(X\le x)\}$. In altre parole $Q_X(p)$ è la \textit{funzione inversa\/} della funzione di ripartizione $F_X(x)=\Pr(X\le x)$.
...
Siano $\Phi_1,\dots,\Phi_n$ evnti \emph{mutuamente esclusivi\/} ed \emph{esaustivi} (la loro unione è tutto $\Omega$) di probabilità $\neq0$.
Il fatto seguente si chiama \emph{Teorema delle Probabilità Totali.}


\begin{theorem}
   Sia $C$ \`e un qualsiasi altro evento, allora
  \ceq{\hfill \Pr(C)}{=}{\sum^n_{i=1} \Pr(\Phi_i)\cdot \Pr\big(C| \Phi_i).}
\end{theorem}
...
\begin{proof}
  ...
  \qquad$\Pr(C)\ \ =\ \ \Pr(\Phi_1)\cdot \Pr\big(C| \Phi_1\big) + \Pr(\Phi_2)\cdot \Pr\big(C| \Phi_2\big)$

  Applichiamo la definizione di probabilità condizionata.

  \qquad$\displaystyle\Pr(C)\ \ =\ \ \Pr(\Phi_1)\cdot \frac{\Pr\big(C\cap \Phi_1\big)}{\Pr\big(\Phi_1\big)} + \Pr(\Phi_2)\cdot \frac{\Pr\big(C\cap \Phi_2\big)}{\Pr\big(\Phi_2\big)}\ \ =\ \ Pr\big(C\cap \Phi_1\big)\ +\ \Pr\big(C\cap \Phi_2\big)$

  As $C\cap\Phi_1$ and  $C\cap\Phi_2$ are disjoint

  \qquad$\displaystyle\Pr(C)\ \ =\ \ \Pr\big((C\cap \Phi_1)\ \cup\ (C\cap \Phi_2)\big)\ \ =\ \ \Pr\big(C\cap (\Phi_1\cup\Phi_2)\big)\ \ =\ \ \Pr\big(C\cap \Omega\big)\ \ =\ \ \Pr(C)$.
\end{proof}
...
Il fatto seguente si chiama \emph{Teorema (o regola) di Bayes}: per ogni coppia di eventi $A$ e $B$ di probabilit\`a $\neq0$ 
...
\noindent\kern0ex\fbox{\parbox[t]{38ex}{%
...
Un urna contiene dadi a 6 facce e dadi a 4 facce nella proporzione del $30\%$ e $70\%$.
Tutti i dadi sono equilibrati.
Estraiamo un dado dall'urna, lo lanciamo, ed osserviamo il risultato.
Qual'è la probabilità di ottenere un numero tra 1 e 4?
...
\qquad$\displaystyle\Pr\bigg(\bigcup_{i=1}^4E_i\bigg)$.

Applichiamo il Teorema delle Probabilità Totali

\qquad$\displaystyle\Pr\bigg(\bigcup_{i=1}^4E_i\bigg)\quad=\quad
\Pr\bigg(\bigcup_{i=1}^4E_i\ \Big|\ C\bigg)\cdot\Pr\big(C\big)\ +\ 
\Pr\bigg(\bigcup_{i=1}^4E_i\ \Big|\ T\bigg)\cdot\Pr\big(T\big)
$.

\qquad$\displaystyle\phantom{\Pr\bigg(\bigcup_{i=1}^4E_i\bigg)}\quad=\quad
\dfrac16\cdot\dfrac13\ +\ 
1\cdot\dfrac23
$.
