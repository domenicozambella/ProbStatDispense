...
\kern1ex
\fbox{\parbox[t]{41ex}{%
\emph{$T_+\cap H_0$} \textit{corretto positivo}\\

\emph{$\Pr(T_+|H_0)=1{-}\emph{$\beta$}$} \textit{sensibilità}\hfill \emph{potenza\/}
}}


\noindent\kern0ex\fbox{\parbox[t]{38ex}{%
\emph{$T_-\cap H_A$} \ \textit{corretto negativo}\\

\emph{$\Pr(T_-|H_A)=1{-}\emph{$\alpha$}$} \textit{sepecificità}
}}
\kern1ex
\fbox{\parbox[t]{41ex}{%
\emph{$T_-\cap H_0$} \ \textit{falso negativo}\hfill \emph{errore II tipo\/}\\

\emph{$\Pr(T_-|H_0)=\emph{$\beta$}$}
}}
...
La funzione anche aggrega gli errori si chiama \emph{cost function.}
...
Abbiamo {\color{blue}\boldmath\ $\neg T_{+k}=\{0,\dots,12\}$} è la \emph{zona di NON rifiuto}.
...
Per semplificare la discussione dell'errore del II tipo fissiamo $\delta=1/4$ e ci mettiamo nel caso più sfavorevole (il più vicino possibile ad $H_0$)
...
%%%%%%%%%%%%
%%%%%%%%%%%%
\clearpage\subsection{Esempio di domanda in formato esame}
...
%%%%%%%%%%%%
%%%%%%%%%%%%
\clearpage\subsection{Una coda, p-valore}
